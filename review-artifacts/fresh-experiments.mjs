...
const prefix=new URL(`./fresh-${mode}-${seed}`,import.meta.url);
...
const dynamic={calls:0,energyTraceMismatch:0,nonFixedReturns:0,maxTraceError:0,maxQuenchFlips:0,reportedNonConverged:0,quietFallbackEngaged:0,convergenceFlagMismatch:0};
const raw=ex.mem.net.settleAnnealed.bind(ex.mem.net);
ex.mem.net.settleAnnealed=(input,wells,opts)=>{
  const clamp=new Set(input);const r=raw([...clamp],wells,opts);dynamic.calls++; if(!r.converged)dynamic.reportedNonConverged++; if(r.converged && r.residualFlips!==0)dynamic.convergenceFlagMismatch++; if(opts?.fallbackQuietOnly && !r.converged && ex.mem.net.diDriveEngaged())dynamic.quietFallbackEngaged++;
