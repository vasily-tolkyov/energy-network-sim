...
async function rareRun(policy: "uncertainty" | "balanced", budget: number, seed: number) {
...
async function thresholdRun(policy: "uncertainty" | "balanced", budget: number, seed: number) {
...
async function circuitRun(policy: "uncertainty" | "balanced", budget: number, seed: number) {
...
for (const seed of seeds) {
  u1.uncertainty.push(await rareRun("uncertainty", 32, seed));
  u1.balanced.push(await rareRun("balanced", 32, seed));
  u1.random.push(rareRandom(32, seed));
  const u = u1.uncertainty.at(-1)! as Awaited<ReturnType<typeof rareRun>>;
  const b = u1.balanced.at(-1)! as Awaited<ReturnType<typeof rareRun>>;
...
for (const seed of seeds) {
  u3.uncertainty.push(await thresholdRun("uncertainty", 120, seed));
  u3.balanced.push(await thresholdRun("balanced", 120, seed));
  const u = u3.uncertainty.at(-1)! as Awaited<ReturnType<typeof thresholdRun>>;
  const b = u3.balanced.at(-1)! as Awaited<ReturnType<typeof thresholdRun>>;
...
const u2 = { uncertainty: await circuitRun("uncertainty", 150, 1), balanced: await circuitRun("balanced", 150, 1) };
