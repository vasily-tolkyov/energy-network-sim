...
async function runWorld(world: MultistageWorld, seed: number) {
  const t0 = performance.now();
  const before = heapMB();
  const model = new TransitionMemory(world.space);
  const bench = new MultistageBench(world);
  let t = performance.now();
  const collection = await collectTransitions(model, bench, collectBudget(world), seed);
  ...
  for (const task of world.tasks) {
    ...
    tasks.push({ planStatus: plan.status, planSteps: plan.steps.length, planMs: +planMs.toFixed(0),
...
async function runWorldFlip(seed: number) {
  const model = new TransitionMemory(W7.space);
  const bench = new MultistageBench(W7);
  await collectTransitions(model, bench, collectBudget(W7), seed);
  ...
  const exec = await executeGoal(model, countingFlippingBench, W7.tasks[0]!.start, goal, seed);
  // 翻转后：失配检测存在；捷径经失配观察学到，重规划绕行到达
  const detected = exec.steps.some(s => s.capture.class !== "within-envelope");
  // 再探索：同一记忆在翻转后的世界重新收集
  await collectTransitions(model, { conduct: (s, a) => flippingBench.conduct(s, a) } , collectBudget(W7) * 2, seed);
...
for (const seed of seeds) {
  for (const world of [W6, W8]) rows.push(await runWorld(world, seed));
  (report as any).w6w8 = rows;
}
(report as any).w7 = []; for (const seed of seeds) (report as any).w7.push(await runWorldFlip(seed));
...
for (const r of (report as any).w7 as Awaited<ReturnType<typeof runWorldFlip>>[]) {
