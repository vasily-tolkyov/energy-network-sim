...
function trained(seed: number): TransitionMemory {
  const model = new TransitionMemory(CHAIN_SPACE);
  collectTransitions(model, new OrderedChainBench(), 40, seed);
  return model;
}
...
for (const seed of [1, 2, 3]) {
  test(`chain seed ${seed}: ordered route and no reverse door`, () => {
    const model = trained(seed);
    const execution = executeGoal(model, new OrderedChainBench(), { node: 0 }, { node: 4 }, seed);
  ...
  test(`chain seed ${seed}: gate closure triggers replanning and an honest stop at C`, () => {
    const model = trained(seed);
    const bench = new OrderedChainBench();
    ...
    const execution = executeGoal(model, injecting, { node: 0 }, { node: 4 }, seed);
  ...
  test(`chain seed ${seed}: displaced B→D replans from D and preserves the old snapshot`, () => {
    const model = trained(seed);
    const bench = new OrderedChainBench();
    let injected = false;
